...
export function QuizzSaved() {
  ...
  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem('tests')) || { preguntas: [], respuestas: [], numero_pregunta: 0 };
    setData(stored);
  ...
  const handleAnswer = (answer) => {
    answers[questionNumber] = answer.toString();
    setAnswers([...answers]);
  }
  ...
  const handlerGuardarTest = () => {
    ...
    localStorage.setItem('tests', JSON.stringify(object));
  }
  ...
  if (finish === true) {
    ...
    if (data.length === 0) {
      return <h1>No hay preguntas guardadas</h1>;
    } else {
      return (
        <div>
